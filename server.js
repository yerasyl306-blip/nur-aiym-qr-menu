...
], tables:[1,2,3,4,5,6,7,8], orders:[], categories:['Ыстық тағамдар','Салаттар','Нан өнімдері','Сусындар'], printJobs:[] };
function data(){
  try{
    const saved=JSON.parse(fs.readFileSync(store,'utf8'));
    return {...structuredClone(initial),...saved,foods:Array.isArray(saved.foods)?saved.foods:structuredClone(initial.foods),orders:Array.isArray(saved.orders)?saved.orders:[],tables:Array.isArray(saved.tables)?saved.tables:structuredClone(initial.tables),categories:Array.isArray(saved.categories)?saved.categories:structuredClone(initial.categories),printJobs:Array.isArray(saved.printJobs)?saved.printJobs:[]};
  }catch{return structuredClone(initial)}
}
function save(value){fs.writeFileSync(store,JSON.stringify(value,null,2),'utf8')}
function json(res,status,value){res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});res.end(JSON.stringify(value))}
function read(req){return new Promise((resolve,reject)=>{let body='',tooLarge=false;req.on('data',c=>{body+=c;if(body.length>8000000){tooLarge=true;req.destroy()}});req.on('end',()=>{if(tooLarge)return reject();try{resolve(body?JSON.parse(body):{})}catch{reject()}});req.on('error',reject)})}
...
http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/api/orders'&&req.method==='POST'){try{const body=await read(req),db=data();const items=(Array.isArray(body.items)?body.items:[]).map(line=>{const food=db.foods.find(f=>f.id===Number(line.id)&&!f.stopped),qty=Math.max(1,Math.min(50,Number(line.qty)||1));return food&&{id:food.id,name:food.name,price:food.price,qty}}).filter(Boolean);if(!items.length)return json(res,400,{ok:false,error:'Себет бос.'});const type=['table','pickup','delivery'].includes(body.type)?body.type:'pickup',table=String(body.table||'');const customer=String(body.customer||'').trim().slice(0,80),phone=String(body.phone||'').trim().slice(0,30),address=type==='delivery'?String(body.address||'').trim().slice(0,180):'';if(type==='table'&&!db.tables.map(String).includes(table))return json(res,400,{ok:false,error:'Үстел нөмірі қате.'});if(type==='delivery'&&(!customer||!phone||!address))return json(res,400,{ok:false,error:'Жеткізу үшін аты-жөні, телефон және мекенжай қажет.'});const order={id:Date.now(),number:(db.orders.reduce((max,o)=>Math.max(max,Number(o.number)||0),0)+1),type,table:type==='table'?table:'',customer,phone,address,items,total:items.reduce((s,i)=>s+i.price*i.qty,0),status:'Жаңа',date:new Date().toLocaleString('kk-KZ')};db.orders.unshift(order);db.printJobs.unshift({id:crypto.randomUUID(),createdAt:Date.now(),order,leasedUntil:0});save(db);event('new-order',order);return json(res,201,{ok:true,order})}catch{return json(res,400,{ok:false,error:'Тапсырыс сақталмады.'})}}
  ...
  if(url.pathname==='/api/print-jobs'&&req.method==='GET'){
    if(!process.env.PRINTER_KEY||req.headers['x-printer-key']!==process.env.PRINTER_KEY)return json(res,401,{ok:false});
    const db=data(),now=Date.now(),jobs=db.printJobs.filter(job=>(job.leasedUntil||0)<now).slice(0,10);
    jobs.forEach(job=>job.leasedUntil=now+60000); if(jobs.length)save(db);
    return json(res,200,{jobs});
  }
  if(url.pathname.startsWith('/api/print-jobs/')&&url.pathname.endsWith('/ack')&&req.method==='POST'){
    if(!process.env.PRINTER_KEY||req.headers['x-printer-key']!==process.env.PRINTER_KEY)return json(res,401,{ok:false});
    const id=url.pathname.split('/')[3],db=data(),before=db.printJobs.length;db.printJobs=db.printJobs.filter(job=>job.id!==id);if(before===db.printJobs.length)return json(res,404,{ok:false});save(db);return json(res,200,{ok:true});
  }
